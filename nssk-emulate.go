...
func main() {
	...
	master := flag.String("master", "http://localhost:5000", "中央主机访问地址")
	flag.Parse()
...
func run(address string, master string) bool {
	if master != address {
		...
		resp, err := requests.Post(master+"/api-master/user/join", map[string]interface{}{"address": address})
		if err != nil {
			return false
		}
		secret := resp.JSON()["body"]
